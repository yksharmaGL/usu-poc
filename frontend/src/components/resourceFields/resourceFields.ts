import { Components, Formio, Utils as FormioUtils } from '@formio/js';
...
export default class ResourceFieldsComponent extends NestedComponent {
  ...
  addDynamicFields() {
    ...
    Formio.request(`${Formio.getProjectUrl()}/form/${this.data.resource}`).then((result: any) => {
      ...
      FormioUtils.eachComponent(this.options.currentForm.components, (component: any, path: any) => {
      ...
      FormioUtils.eachComponent(result.components, (component: any, path: any) => {
        if (component.type !== 'button' && path.indexOf('.') === -1) {
